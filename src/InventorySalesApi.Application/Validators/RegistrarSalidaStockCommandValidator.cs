...
public class RegistrarSalidaStockCommandValidator : AbstractValidator<RegistrarSalidaStockCommand>
{
    public RegistrarSalidaStockCommandValidator()
    {
        RuleFor(x => x.ProductoId)
            .NotEmpty().WithMessage("El ID del producto es requerido.");

        RuleFor(x => x.Cantidad)
            .GreaterThan(0).WithMessage("La cantidad a egresar debe ser mayor a cero.");
